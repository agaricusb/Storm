...
public class ReflectConfiguration {
    ...
    public void load() {
        try {
            synchronized (this) {
                plugin.getLogger().log(Level.CONFIG, "Loading configuration file: " + name);
                onLoad(plugin);
            }
        } catch (Exception e) {
            ...
            for (Field field : getClass().getDeclaredFields()) {
                ...
                if (worlds.isSet(path)) {
                    ...
                    if ((lim = field.getAnnotation(LimitInteger.class)) != null) {
                        ...
                        if (doCorrect) {
                            plugin.getLogger().log(Level.CONFIG, lim.warning().replace("%node", "'" + path.substring(6) + "'").replace("%limit", limit + ""));
                            if (lim.correct())
