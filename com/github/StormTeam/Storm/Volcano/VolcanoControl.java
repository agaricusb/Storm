...
public class VolcanoControl implements Listener {
    ...
    public void coolLava(BlockFromToEvent e) {
        ...
        for (VolcanoMaker volcano : volcanoes) {
            if ((from.getTypeId() & 0xfe) == 0xa && volcano.ownsBlock(from)) //Checks if the block is lava and if a volcano owns it
    ...
    public void unloadWorld(WorldUnloadEvent e) {
        World unloaded = e.getWorld();
        for (VolcanoMaker maker : volcanoes) {
            if (maker.getWorld().equals(unloaded))
                volcanoes.remove(maker);
        }
    }
